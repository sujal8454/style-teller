...
# --- Logo Base64 Constant (High-resolution image embedded) ---
LOGO_BASE64 = ""
...
def get_img_as_base64(base64_string):
    ...
    try:
        # Clean the base64 string by removing common prefixes
        if ',' in base64_string:
...
# --- Custom Styling and Animations (Fulfills all 3 requests) ---

# This CSS block implements the smooth transitions and fixes the video overlay.
page_fadein_css = """
<style>
/* 2. Page-Load Transition: Gentle fade-in for entire page content (1 second) */
/* Target the main application container to apply the initial fade-in */
/* The animation starts only after the video is done playing and the 'video-active' class is removed. */
div[data-testid="stAppViewContainer"] > div:first-child {
    opacity: 0; 
    animation: fadeInPage 1.0s ease-out forwards; 
}
...
/* 3. Logo Placement & Animation: Centered, proportionate logo with synchronized fade-in */
.logo-container {
    display: flex;
    ...
    /* Synchronized with page fade-in (1.5s total duration with slight delay for cohesive look) */
...
/* 1. Intro Video Fix: Smooth fade-in (1-2 seconds) and removal of black overlay/visibility issue */
/* This class is added to the body element via the JS helper below */
.video-active div[data-testid="stVideo"] {
    ...
    opacity: 0; 
    animation: fadeInVideo 1.5s ease-out 0.1s forwards; /* 1.5 second fade-in for premium feel */
}

/* Ensure the video element itself covers the container */
.video-active div[data-testid="stVideo"] video {
    width: 100vw !important;
    height: 100vh !important;
    object-fit: cover !important; /* Ensures video covers the entire screen without distortion */
}

/* FIX: Hides the main app content while the video is full screen/playing */
...
/* General Styling for white background/black text consistency */
...
<script>
// JS Helper to toggle the 'video-active' class on the body for full-screen video
...
def header():
    """
    Displays the application header and logo. (Fulfills Request 3 placement and animation)
    """
    
    logo_bytes = get_img_as_base64(LOGO_BASE64)
    
    # 3. Logo Placement: Insert the logo at the top center using the animated CSS class
    st.markdown("<div class='logo-container'>", unsafe_allow_html=True)
    if logo_bytes:
        # Using a width of 180px for high-resolution, proportionate display
        st.image(logo_bytes, width=180, caption="") 
    else:
...
def sign_out():
    ...
    # Reset video state for next launch
    st.session_state["video_played"] = False 
...
def intro_video():
    """
    Plays the introductory video, ensuring smooth playback and handling transitions.
    (Fulfills Request 1)
    """
    
    # 1. Activate fullscreen mode CSS via JS (Fixes black overlay/visibility issue)
    ...
    # 3. Display the video, using 'autoplay=True' for smooth start
    if video_bytes:
        st.video(video_bytes, format="video/mp4", start_time=0, autoplay=True) 

        # 4. Auto-transition logic (based on expected duration)
        if "video_start_time" not in st.session_state:
            st.session_state["video_start_time"] = time.time()

        VIDEO_DURATION_SECONDS = 7 # Expected duration
        time_elapsed = time.time() - st.session_state["video_start_time"]
        
        if time_elapsed < VIDEO_DURATION_SECONDS:
             # Use st.empty to show a temporary loading message during the video playback
             with st.empty():
...
def main():
    ...
    # This runs on every page that isn't the video page to ensure the app UI is visible
    st.components.v1.html("<script>setVideoMode(false);</script>", height=0)
